...
import { BuiltBy } from "@/components/hero/built-by";
import { Container } from "@/components/ui/section";
import { siteCopy } from "@/content/copy";
import { track } from "@/lib/analytics";
import { EASE_ENTER, EASE_SWEEP } from "@/lib/motion";
 ...
 * so there is nothing to show through.
 *
 * Two earlier revisions got this wrong in instructive ways. A crossfade between
 * the two glyphs put half an R and half an O in the same place at once, which is
 * a smudge. A vertical roll made each seat a two row reel, which is mechanically
 * correct but composites the bottom of one letter with the top of the other, and
 * the reader sees a shape that is neither.
 *
 * The cuts are derived from the line rather than animated alongside it, so the
 * two cannot drift apart on any frame, at any width.
 *
 * WHY NOTHING MOVES
 *
...
export function Hero() {
  ...
  /** True once the word has changed: the animation is over and the word is a word. */
  ...
  useEffect(() => {
    ...
    /** The copy arriving. Runs immediately, never behind the sweep. */
    const reveal = async () => {
    ...
    const run = async () => {
      // The copy goes up straight away. Holding it back until the pass finished
      // left the page looking broken for the first three seconds, and hid the
      // form behind an animation nobody had agreed to wait for.
      await reveal();
      if (cancelled) {
        return;
      }
      ...
      await Promise.all([
        animate(progress, 1, {
          duration: (reduceMotion ? 1500 : SWEEP_MS) / 1000,
          ease: SWEEP_EASE,
        }),
        animate(
          "[data-beam-line]",
  ...
  return (
    ...
    >
      <Container>
        ...
        {/* The word is painted from the first frame rather than revealed: it is
            the largest thing on the page and the line has to cross a word the
            visitor can already see. */}
        <div
          aria-hidden="true"
          className="mt-3 font-display font-semibold sm:mt-4"
        >
          <span className="hero-word text-display text-ink" data-word>
            {SEATS.map((seat) => {
              ...
              return (
                <span className="hero-seat" data-slot={seat.id} key={seat.id}>
                  {seat.glyphs.map((char, index) => {
                    ...
                    // `motion.span` throughout so the two letters that move can
                    // carry a motion value. The rest are given none, so Motion
                    ...
                    return (
                      <motion.span
                        ...
                        style={
                          swapped
                            ? arriving
                              ? // Stated rather than left off. Motion owns the
                                // inline clip while the seats are animating, and
                                // dropping the prop leaves its last value behind,
                                // so the settled state says what it means.
                                { clipPath: SHOWN }
                              : replaced
                                ? { visibility: "hidden" }
                                : undefined
            ...
            {/* The line, carried across the word by one translated element. It
                may not be wrapped in anything that isolates its blending, or it
                would vanish over a stroke. The carrier is left where the
                crossing ended rather than being parked back: the cuts are read
                from the same value, so resetting it would un-reveal the
                letters. */}
        ...
        <div className="mt-8 sm:mt-10">
          ...
          <motion.p
            className="mt-4 max-w-measure text-base text-ink-muted"
            data-reveal
            initial={{ opacity: 0, y: 14 }}
          >
            <BuiltBy />
